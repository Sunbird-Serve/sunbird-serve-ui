...
function EventsSideBar(props) {
    ...
    const todayEvents = mockEvents.filter( x => dayjs(x.start).isSame(dayjs(selectedDate),"day"))

    return (
        <div className="events">
            <div className = 'title'>Needs | {new Date(selectedDate).toLocaleDateString('en-US', options)}</div>
            <div>
            {console.log("sidebar", todayEvents)}
            {todayEvents.length === 0
                ?
                    <div style = {{height: '80vh', display:'flex', flexDirection:'column'}}>
                        <EventBusyIcon style = {{fontSize: '150px', margin:'auto auto 0 auto', color: '#c4c4c4'}}/>
                        <span style = {{margin:'0 auto auto auto'}}>No event on this day</span>
                    </div>
                :
