...
export const fetchEntityNeeds = createAsyncThunk(
  ...
  async () => {
    try {
      const response1 = await axios.get(
        `${configData.ENTITY_NEEDS}/?page=0&size=100&status=New`
      );
      const response2 = await axios.get(
        `${configData.ENTITY_NEEDS}/?page=0&size=100&status=Nominated`
      );
      const response3 = await axios.get(
        `${configData.ENTITY_NEEDS}/?page=0&size=100&status=Approved`
      );
      const response4 = await axios.get(
        `${configData.ENTITY_NEEDS}/?page=0&size=100&status=Rejected`
      );
      const response5 = await axios.get(
        `${configData.ENTITY_NEEDS}/?page=0&size=100&status=Assigned`
      );
      const response6 = await axios.get(
        `${configData.ENTITY_NEEDS}/?page=0&size=100&status=Fulfilled`
      );
      return [
        ...response1.data.content,
        ...response2.data.content,
